...
class GenericCollection <T extends Comparable<T> & IHasTimestamp>{
    ...
    void addGenericItem(String category, T element) {
        ...
        map.get(category).sort(Comparator.reverseOrder());
    }


    Collection<T> findAllBetween (LocalDateTime from, LocalDateTime to) {
        Predicate<T> condition = t -> t.getTimestamp().isAfter(from) && t.getTimestamp().isBefore(to);
...
//        Predicate<T> condition = t -> categories.stream().anyMatch(c -> map.get(c).contains(t));
//        return map.values().stream().flatMap(Collection::stream)
//                .filter(condition)
//                .sorted(Comparator.reverseOrder())
//                .collect(Collectors.toCollection(ArrayList::new));
...
//    }
    ...
    Collection<T> itemsFromCategories(List<String> categories) {
        return categories.stream()
                ...
                .flatMap(c -> map.get(c).stream())
                .sorted(Comparator.reverseOrder())
                .collect(Collectors.toList());
    }


    public String monthAndDay(T e) {
        return String.format("%02d-%02d",
                e.getTimestamp().getMonthValue(), e.getTimestamp().getDayOfMonth());
    }


    public int getYear(T e) {
        return e.getTimestamp().getYear();
    }


    public Map<String, Set<T>> byMonthAndDay() {
        Map<String, Set<T>> result = map.values().stream()
        ...
        result.values()
                .forEach(set -> {
                    Set<T> sorted = new TreeSet<>(Comparator.reverseOrder());
                    sorted.addAll(set);
                    set.clear();
                    set.addAll(sorted);
                });
